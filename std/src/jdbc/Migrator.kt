...
class Migrator(location: File) {
    ...
    private val log = logger("jdbc")

    init {
        runBlocking { executeSql(Resource.read("jdbc/migrations.sql")) }
    }
            ...
            .mapNotNull { (candidate, migration) ->
                try {
                    transaction {
                        ...
                        log.info("Migrated ${candidate.file.name} successfully")
                    }
                } catch (e: Exception) {
                    log.info("Migration of ${candidate.file.name} failed")
